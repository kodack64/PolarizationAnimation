...
int meshCountZ = (int)(5 * waveLength / meshInterval); // num
int meshCountX = 10; // num
double meshZShift = -waveLength*1.5; // (nm)

// arrows
//double arrowInterval = waveLength / 18; //(nm)
double arrowInterval = waveLength / 30; //(nm)
int arrowCount = (int)(meshCountZ * 2 * meshInterval / arrowInterval); // num
double arrowHeight = waveLength / 1; // (nm)
double arrowRadius = arrowInterval / 6; // (nm)
...
void procCommand() {
	...
	if (command == "record") {
		...
		vw = cvCreateVideoWriter("cap.avi", -1, fps - 0.03, cvSize(width, height));
		recordFlag = true;
		recordCount = 0;
	}
